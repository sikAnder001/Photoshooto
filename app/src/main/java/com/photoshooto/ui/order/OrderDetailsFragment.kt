...
import com.photoshooto.databinding.FragmentOrderDetailsBinding
...
class OrderDetailsFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        data = requireArguments().getParcelable("item")!!
        initView()
    }
    ...
    private fun initView() {
        ...
        if (data.status == REJECT || data.status == DECLINE || data.status == BLOCK) {
    ...
    private fun handleProceedClick() {
        ...
        if (billId.isEmpty()) {
            ...
            return
            /* if (Character.isDigit(billId[0]) && !billId.isValidMobileNumber()) {
                 binding.editBillId?.requestFocus()
                 binding.erroreEditBillId?.text = getString(R.string.enter_mobile_empid)
                 binding.erroreEditBillId?.visibility = View.VISIBLE
             } else if (!Character.isDigit(billId[0]) && !isValidEmail(billId)) {
                 binding.editBillId?.requestFocus()
                 binding.erroreEditBillId?.text = getString(R.string.enter_valid_email)
                 binding.erroreEditBillId?.visibility = View.VISIBLE
             }*/

            // userData.username = billId
            //  userData.password = status
        ...
        dialogBinding.apply {
            ...
            (recyclerView.adapter as CommonSingleSelectAdapter).onItemClickListener =
                object : CommonSingleSelectAdapter.OnItemClickListener {
                    override fun onDetailsClick(data: String) {
                        binding.etStatus.setText(data)
